...
private val PREFIX = "relay-registration"
...
fun main(args: Array<String>) {
    ...
    val relayRegistrationConfig =
        loadConfigs(PREFIX, RelayRegistrationConfig::class.java, "/eth/relay_registration.properties")
    val passwordConfig = loadEthPasswords(PREFIX, "/eth/ethereum_password.properties", args)
    val credentials =
        loadConfigs(PREFIX, RelayRegistrationCredentials::class.java, "/eth/relay_registration_credentials.properties")

    IrohaInitialization.loadIrohaLibrary()
        .flatMap { RelayRegistration(relayRegistrationConfig, passwordConfig, credentials).deploy() }
        .failure { ex ->
